...
public class InputManager
{
    ...
    public BattleBlockData GetMouseOverAnyBattleBlock() // 추가 : 마우스오버 동작
    {
        Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
    ...
    public BattleBlockData GetMouseRHoldAnyBattleBlock(int mouseButton)  // 추가 : 마우스우클릭 동작
    {
        if (Input.GetMouseButton(mouseButton))// && !IsPointerOverUIObjectExceptTouchpad() && !EventSystem.current.IsPointerOverGameObject())
        {
            Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
