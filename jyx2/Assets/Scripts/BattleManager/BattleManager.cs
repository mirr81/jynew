...
namespace Jyx2
{
    ...
    public class BattleManager : MonoBehaviour
    {
        ...
        public async UniTask StartBattle(BattleStartParams customParams)
        {
            ...
            //特定位置的翻译【战斗开始时候的弹窗提示】
            //---------------------------------------------------------------------------
            await Jyx2_UIManager.Instance.ShowUIAsync(nameof(CommonTipsUIPanel), TipsType.MiddleTop, "전투 시작".GetContent(nameof(BattleManager))); //提示UI
            //---------------------------------------------------------------------------
            //---------------------------------------------------------------------------
        ...
        public void OnBattleEnd(BattleResult result)
        {
            ...
            switch (result)
            {
                case BattleResult.Win:
                {
                    ...
                    //特定位置的翻译【战斗胜利的提示】
                    //---------------------------------------------------------------------------
                    GameUtil.ShowFullSuggest(bonusText, "<color=yellow>전투 승리</color>".GetContent(nameof(BattleManager)), delegate
                    {
                        EndBattle();
                        m_battleParams.callback?.Invoke(result);
                        m_battleParams = null;
                ...
                case BattleResult.Surrender:
                {
                    ...
                    //特定位置的翻译【战斗失败的提示】
                    //---------------------------------------------------------------------------
                    GameUtil.ShowFullSuggest("승패는 병가의 상사이니 다시 도전하자。".GetContent(nameof(BattleManager)), "<color=red>전투 패배！</color>".GetContent(nameof(BattleManager)), delegate
                    {
                        EndBattle();
                        m_battleParams.callback?.Invoke(result);
                        //if (m_battleParams.backToBigMap) //由dead指令实现返回主界面逻辑
        ...
        string CalExpGot(LBattleConfig battleData)
        {
            ...
            foreach (var role in all_teammates)
            {
                ...
                if (practiseItem != null)
                {
                    ...
                    if ((int)practiseItem.ItemType == 5) {
                        bonusTextBuilder.AppendFormat("{0} 이 훼손되어 더 이상 사용할 수 없습니다.\n".GetContent(nameof(BattleManager)), practiseItem.Name);
                        GameRuntimeData.Instance.AddItem(practiseItem.Id, -1);
                        role.ExpForItem = 0;
                        role.Xiulianwupin = -1;
                        GameRuntimeData.Instance.SetItemUser(role.Xiulianwupin, -1);
                    }
                }
            }
        ...
        BattleBlockData FindNearestBattleBlock(Vector3 pos, bool ignoreRole = false)
        {
            ...
            //先检查脚下的格子是否能用，如果能用就不再遍历查找最近格子  먼저 발밑의 칸을 사용할 수 있는지 검사하고, 사용할 수 있다면 더 이상 가장 가까운 칸을 찾아다니지 않는다
