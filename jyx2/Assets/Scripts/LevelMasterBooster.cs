...
public class LevelMasterBooster : MonoBehaviour
{
    ...
    [LabelText("전투지도")] public bool m_IsBattleMap = false;
    ...
    private const string QUEST_SCENE_INFO_PRFIX = "quest:";
    ...
    /// <param name="scene">비어 있는 경우 현재 화면</param>
    ...
    public async UniTask RefreshSceneObjects()
    {
        ...
        foreach(var kv in dict)
        {
            ...
            else if (kv.Value.StartsWith(QUEST_SCENE_INFO_PRFIX))
            {   
                //Debug.Log($"RefreshSceneObjects : {kv.Key}, origin : {obj.transform.position}, target : {kv.Value}");
                obj.SetActive(true);
                var trans = obj.transform;
                string posStr = kv.Value.Replace(QUEST_SCENE_INFO_PRFIX, "");
                string[] posVec = posStr.Split(',');
                float x = float.Parse(posVec[0]);
                float y = float.Parse(posVec[1]);
                float z = float.Parse(posVec[2]);
