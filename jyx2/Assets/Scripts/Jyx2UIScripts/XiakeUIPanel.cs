...
public partial class XiakeUIPanel : Jyx2_UIBase
{
    ...
    string GetInfoText(RoleInstance role)
    {
        ...
        //---------------------------------------------------------------------------
        sb.AppendLine(string.Format("등급 {0}".GetContent(nameof(XiakeUIPanel)), role.Level));
        sb.AppendLine(string.Format("체력 {0}/{1}".GetContent(nameof(XiakeUIPanel)), role.Tili, GameConst.MAX_ROLE_TILI));
        sb.AppendLine(string.Format("생명 <color={0}>{1}</color>/<color={2}>{3}</color>".GetContent(nameof(XiakeUIPanel)), color1, role.Hp, color2,
            role.MaxHp));
        sb.AppendLine(string.Format("내력 <color={0}>{1}/{2}</color>".GetContent(nameof(XiakeUIPanel)), color, role.Mp, role.MaxMp));
        sb.AppendLine(string.Format("경험 {0}/{1}".GetContent(nameof(XiakeUIPanel)), role.Exp, role.GetLevelUpExp()));
        sb.AppendLine();
        sb.AppendLine(string.Format("공격 {0}".GetContent(nameof(XiakeUIPanel)), role.Attack));
        sb.AppendLine(string.Format("방어 {0}".GetContent(nameof(XiakeUIPanel)), role.Defence));
        sb.AppendLine(string.Format("경공 {0}".GetContent(nameof(XiakeUIPanel)), role.Qinggong));
        sb.AppendLine(string.Format("의술 {0}".GetContent(nameof(XiakeUIPanel)), role.Heal));
        sb.AppendLine(string.Format("해독 {0}".GetContent(nameof(XiakeUIPanel)), role.DePoison));
        sb.AppendLine(string.Format("중독 {0}".GetContent(nameof(XiakeUIPanel)), role.UsePoison));
        sb.AppendLine();
        sb.AppendLine(string.Format("권장 {0}".GetContent(nameof(XiakeUIPanel)), role.Quanzhang));
        sb.AppendLine(string.Format("검술 {0}".GetContent(nameof(XiakeUIPanel)), role.Yujian));
        sb.AppendLine(string.Format("도술 {0}".GetContent(nameof(XiakeUIPanel)), role.Shuadao));
        sb.AppendLine(string.Format("특수 {0}".GetContent(nameof(XiakeUIPanel)), role.Qimen));
        sb.AppendLine(string.Format("암기 {0}".GetContent(nameof(XiakeUIPanel)), role.Anqi));
        //---------------------------------------------------------------------------
        //---------------------------------------------------------------------------
        if (GameSettingManager.GetDifficulty() == (int) Jyx2_GameDifficulty.Simple)
            sb.AppendLine(string.Format("자질 {0}".GetContent(nameof(XiakeUIPanel)), role.IQ));
    ...
    void GetSkillList(RoleInstance role)
    {
        var num = 0;
        foreach (var skill in role.Wugongs)
        {   
            StringBuilder sb = new StringBuilder();
            sb.AppendLine(skill.Name + " " + skill.GetLevel());
            SkillList_Text[num].text = sb.ToString();
            num++;
        }
        for (int i=num; i < GameConst.MAX_SKILL_COUNT; i++) 
        {
            SkillList_Text[i].text = "";
        }
    }
    ...
    string GetItemsText(RoleInstance role)
    {
        ...
        sb.AppendLine("무기：".GetContent(nameof(XiakeUIPanel)) + (weapon == null ? "" : weapon.Name));

        var armor = role.GetArmor();
        sb.AppendLine("방어구：".GetContent(nameof(XiakeUIPanel)) + (armor == null ? "" : armor.Name));

        var xiulianItem = role.GetXiulianItem();
        sb.AppendLine("수련：".GetContent(nameof(XiakeUIPanel)) + (xiulianItem == null
            ? ""
    ...
    void OnLeaveClick()
    {
        ...
        {
            GameUtil.DisplayPopinfo("파티를 떠나기 전에 대지도에 있어야 합니다");
            return;
        }
        ...
        {
            GameUtil.DisplayPopinfo("주인공은 파티를 떠날 수 없습니다");
            return;
        }
        ...
        MessageBox.ConfirmOrCancel("이 인물을 탈퇴시키겠습니까?", DoCharacterLeaveTeam, onCanceled);
    }
    ...
    async void OnHealClick()
    {
        ...
        selectParams.title = "치료가 필요한 사람을 선택하십시오";
    ...
    async void OnDetoxicateClick()
    {
        SelectRoleParams selectParams = new SelectRoleParams();
        selectParams.roleList = m_roleList;
        selectParams.title = "해독이 필요한 사람을 선택하십시오";
        selectParams.isDefaultSelect = false;
        selectParams.callback = (cbParam) =>
        {
            this.gameObject.SetActive(true);
