...
namespace Jyx2
{
    public class ArchiveItem:MonoBehaviour,IDataContainer<int>,INavigable
    {
        ...
        void RefreshView()
        {
            ...
            string archiveName = "저장 ".GetContent(nameof(SavePanel));
            m_TitleTxt.text = archiveName + archiveIdxStr;

            var summaryText = GameSaveSummary.Load(m_ArchiveIndex).GetBrief();
            summaryText = string.IsNullOrEmpty(summaryText) ? "빈 공간".GetContent(nameof(SavePanel)) : summaryText;
            m_SummaryTxt.text = summaryText;

            var dateText = GameRuntimeData.GetSaveDate(m_ArchiveIndex)?.ToLocalTime().ToString("yyyy년M월d일 H시m분");
