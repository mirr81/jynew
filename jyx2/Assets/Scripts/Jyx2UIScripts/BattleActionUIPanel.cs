...
public partial class BattleActionUIPanel : Jyx2_UIBase
{
	...
	async UniTask ShowAttackRangeAsync(SkillCastInstance skillCast)
	{
#if UNITY_ANDROID
		// Debug.Log("jiin >> Before Delay");
		await UniTask.Delay(100);
		// Debug.Log("jiin >> After Delay");
#endif
	...
	//显示攻击范围选择指示器	공격 범위 선택 표시기 보이기
	void ShowAttackRangeSelector(SkillCastInstance skillCast)
	{
		ShowAttackRangeAsync(skillCast).Forget();
		// currentSkill = skillCast;
	...
	public override void Update()
	{
		...
		//显示当前攻击范围	현재 공격 범위 보이기
		ShowSkillCastHitRange();

		//寻找玩家点击的格子	유저가 클릭한 칸 찾기
		var block = InputManager.Instance.GetMouseUpBattleBlock();

		//没有选择格子	칸을 선택하지 않았습니다.
		if (block == null) return;

		//格子隐藏（原则上应该不会出现）	체크 숨기기 (원칙적으로 나타나지 않음)
		if (!block.gameObject.activeSelf) return;

		//选择移动，但位置站人了	이동을 선택했지만 위치는 사람이 섰습니다
    ...
    void RefreshSkill()
	{
		...
		// 	var width = Skills_RectTransform.rect.width;
		// 	var spacing = Skills_GridLayoutGroup.spacing.x;
		// 	var cellWidth = m_CacheSkillItems[0].Transform.GetComponent<RectTransform>().rect.width;	// 150			
		// 	(m_CurSkillItems.Count * cellWidth + (m_CurSkillItems.Count - 1) * spacing) > width
		// }
	...
	public void OnSurrenderClick()
	{
		...
		MessageBox.ConfirmOrCancel("항복 : 전투를 포기하시겠습니까?".GetContent(nameof(BattleActionUIPanel)), onConfirm);
	}
