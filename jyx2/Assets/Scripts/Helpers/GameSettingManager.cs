		...
		foreach (Catalog setting in Enum.GetValues(typeof(Catalog)))
		{
			switch (setting)
			{
				...
				case Catalog.MobileMoveMode:
					...
					break;
				case Catalog.RandomEncounterMode:
					result.Add(Catalog.RandomEncounterMode, GetRandomEncounterMode());
					break;
			}
		}
	...
	private static void UpdateSettingRecord(Catalog setting, object value)
	{
		...
		switch (setting)
		{
			...
			case Catalog.MobileMoveMode:
				...
				break;
			case Catalog.RandomEncounterMode:
				Jyx2_PlayerPrefs.SetInt(GameConst.PLAYER_PREF_RANDOM_ENCOUNTER_MODE, (int)value);
				break;
		}
