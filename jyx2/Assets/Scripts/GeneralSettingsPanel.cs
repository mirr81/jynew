...
public class GeneralSettingsPanel : Jyx2_UIBase,ISettingChildPanel
{
    ...
    {
        var setting = gameSetting[GameSettingManager.Catalog.RandomEncounterMode];
        if (setting is int value)
       {
    ...
    private void OnJoyStickTestBtnClick()
    {
        Action onCopy = () =>
        {
            ...
            CommonTipsUIPanel.ShowPopInfo("디버깅 정보가 클립보드에 복사되었습니다.");
        };

        var sb = new StringBuilder(); 
        var allJoySticks = ReInput.controllers.Joysticks;
        
        sb.AppendLine("<color=red>다음은 조이스틱 테스트의 출력 로그이며, 문제가 있는 경우 클릭하여 복사하여 개발자에게 보내주시기 바랍니다.</color>");
        sb.AppendLine();
        foreach (var joyStick in allJoySticks)
        {
            sb.AppendFormat("<color=yellow>[장치:{0}]</color>  <color=green>GUID:{1}</color>\n", joyStick.hardwareName, joyStick.hardwareTypeGuid);
        }
