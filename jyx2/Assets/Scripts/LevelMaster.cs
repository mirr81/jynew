...
public class LevelMaster : MonoBehaviour
{
	...
	void Start()
	{
		...
		if (gameMap != null && !gameMap.Tags.Contains("WORLDMAP"))
		{
			...
			if (!IsInBattle)
			{
				var showName = gameMap.GetShowName();
				if (gameMap.GetShowName() == GlobalAssetConfig.Instance.defaultHomeName)
				{
					var name = GameRuntimeData.Instance.Player.Name + "거처".GetContent(nameof(LevelMaster));
					//---------------------------------------------------------------------------
					//---------------------------------------------------------------------------
					showName = name;
				}
				//显示当前地图名，大地图不用显示
				Jyx2_UIManager.Instance.ShowUIAsync(nameof(CommonTipsUIPanel), TipsType.MiddleTop, showName).Forget();
			}
		}
	...
	public void OnManuelSave(int index = -1)
	{
		if (runtime == null)
		{
			StoryEngine.DisplayPopInfo("<color=red>저장 실패！</color>");
			Debug.LogError("저장 실패！请从GameStart中启动游戏！");
			return;
		}
		...
		StoryEngine.DisplayPopInfo("저장 성공！");
	}
